...
static inline uint8_t __checkCharNum(const char* _src, uint8_t _checknum, char _c, uint8_t _direction, uint8_t _continue);
static inline char* __match_str_normal(tr_match_str_t _src, tr_match_str_t _des);
static inline uint8_t __checkCharacterWithGlobbing(const char _c,char _specflag);
static inline uint8_t __getRepeatByBraces(const char *_src,uint8_t _len,tr_repeat_t *_repeat);
static inline uint8_t __checkOneOfThem(char _c,const char* _val,pat_size_t _len);
#if TINY_REGEX_CONFIG_OR_ENABLE == 1
static inline pat_size_t __checkSubPatNum(const char* _pattern, pat_size_t _plen,pat_size_t *_pos);
#endif
#if TINY_REGEX_CONFIG_SUBITEM_ENABLE == 1
static inline pat_size_t __getItemEndPos(const char* _pattern,pat_size_t _plen);
#endif
 ...
 *        _slen    : length of sources string;when it zero,it will calcualte sources string by strlen()
 *        _pattern : string of regular expression 
 *        _plen    : length of _pattern length
 * res  : return the match string
 */

tr_res_t tregex_match_str(const char* _srcstr, uint32_t _slen, const char* _pattern, pat_size_t _plen)
{
    ...
    while (p_index <= _plen) {
        switch (_pattern[p_index]) {
            ...
            case '\\': {
                ...
                switch (_pattern[p_index]) {
                    ...
                    default:checkmode = TR_MODE_NORMAL;break;
                }
            }break;
            case '.': {
                specchar = _pattern[p_index];
        ...
        // match  ---------
        // when first character of pattern not match,and second character of pattern not repeat
        // it will check the next charcter of _srcstr
        if (misscount == 1 && checkmode != TR_MODE_REPEAT) {
#if TINY_REGEX_CONFIG_SUBITEM_ENABLE == 1
            if(flag & TINYREGEX_FLAG_ITEM == 0)
#endif
            {checkmode = TR_MODE_NULL;misscount++;}
        }
        switch (checkmode) {
            case TR_MODE_NORMAL: {
                ...
                while (s_index < _slen ) {
                    ...
                    else if (startpos != NULL || (flag & TINYREGEX_FLAG_STAT) !=0) {misscount++;break;}
                    s_index++;
                }
                specchar = '\0';
            }break;
            case TR_MODE_GLOBBING: {
                while (s_index < _slen) {
                    if (__checkCharacterWithGlobbing(_srcstr[s_index],specchar) == 0) {
                        if (startpos == NULL) {startpos = &_srcstr[s_index];}
                        s_index++;
                        misscount = 0;
                        break;
                    }
            ...
            case TR_MODE_REPEAT: {
                if (misscount == 0) {
                    ...
                    if((flag & TINYREGEX_FLAG_ITEM) != 0){
                        repeat[curlevel].Num++;
                        if((repeat[curlevel].Max < 0) || (repeat[curlevel].Num < repeat[curlevel].Max)){
    ...
    for (uint32_t i = not;i < _len;i++) {
        switch (_val[i]) {
            ...
            case '\\': {
                ...
                switch(_val[i+1]){
                    case 's':case 'S':case 'w':case 'W':case 'd':case 'D':case '.':{
                        i+=1;
            ...
            default:res = _val[i]==_c?0:1;break;
        }
        if(res == 0){break;}
    }
end:
